...
- Propostas (Vela): tool `send_proposal` (nunca toca o HubSpot — roda antes do gate `deps.writer`, igual `set_goal`). `mailer/gmail.py` (SMTP + app password, uma conta única da empresa; `GMAIL_USER/APP_PASSWORD` vazios = degrada pra só WhatsApp) + `proposals/` (`template.py` Jinja2 sobre `templates/proposal.html` com os tokens do Ledger embutidos, `pdf.py` WeasyPrint). Escrita de alto risco (`propose_send_proposal`/`confirm_send_proposal` em `actions.py`), mas com confirm próprio — `actions.confirm()` é hard-coded pro update_deal, então `_handle_reply_id`/`_free_text_confirmation` despacham por `pending_action.kind` antes de chamar um ou outro. Sem Undo (nada a desfazer depois de enviado). `send_document` em `gateway.py`/`evolution.py` (`/message/sendMedia`) ainda não verificado contra uma instância real, mesmo tratamento que `download_media` teve antes do smoke test.
